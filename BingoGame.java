...
public class BingoGame implements Serializable{
	...
	class BoardMove {
		...
		BoardMove(int r, int c, Integer val) {
			this.r = r;
			this.c = c;
			this.value = val;
		}
	...
	private int[] numbersCalled = new int[75];
	...
	public BoardMove previousBoardMove; 
	private Stack<BoardMove> history = new Stack<BoardMove>();
	...
	public void setRollLetter(char r) {
		rollLetter = r;
	}
	...
	public void display() {
		...
		for(int j = 0; j<numbersCount; j++) {
			if(numbersCalled[j] <=15)
				System.out.printf("%5s %02d", "B",  numbersCalled[j]);
			else if(numbersCalled[j] <=30)
				System.out.printf("%5s %02d", "I",  numbersCalled[j]);
			else if(numbersCalled[j] <=45)
				System.out.printf("%5s %02d", "N",  numbersCalled[j]);
			else if(numbersCalled[j] <=60)
				System.out.printf("%5s %02d", "G",  numbersCalled[j]);
			else
				System.out.printf("%5s %02d", "O",  numbersCalled[j]);
			System.out.printf(System.lineSeparator());
		}
	}//end display
	...
	public void setPosition(int row, int col) {
		if(!won && positionNotChanged(row, col) ) {
			this.previousBoardMove = new BoardMove(row, col, board[row][col]);
			history.addElement(new BoardMove(row,col,board[row][col]));
			board[row][col] = 0;
			won = gameWon();
		}
	}
	...
	public void roll() {
		...
		numbersCalled[numbersCount] = rollNumber;
		numbersCount++;
	...
	public boolean undo() {
		if(this.previousBoardMove == null) {
			return false;
		}
		this.board[this.previousBoardMove.getR()][this.previousBoardMove.getC()] = this.previousBoardMove.getV();
